...
// Begin the query without a timestamp filter
$sql = "SELECT * FROM sales_records WHERE 1=1 "; // This means we start with no filters
$params = [];
$typeString = "";  // To store the types of the parameters

// Apply search filters based on user input
if ($searchDate) {
    $sql .= "AND DATE(sale_date) = ? ";
    $params[] = $searchDate;
    $typeString .= "s"; // 's' for string
} elseif ($searchMonth && $searchYear) {
    $sql .= "AND YEAR(sale_date) = ? AND MONTH(sale_date) = ? ";
    $params[] = $searchYear;
    $params[] = $searchMonth;
    $typeString .= "ss"; // 'ss' for two strings
} elseif ($searchYear) {
